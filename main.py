...
def normalize_phone(phone):
    """Normalize phone number to standard local format"""
    phone = re.sub(r'[\s\-]', '', str(phone))
...
def init_rbac_database():
    """Initialize database with complete RBAC schema and indexes"""
    ...
    # Insert default branch executives
    branch_execs = [
...
def get_email_settings():
    ...
    if settings:
        try:
            decrypted_password = decrypt_password(settings[4]) if settings[4] else ""
        except:
            decrypted_password = ""
        return {
...
def delete_user(user_id, deleted_by):
    ...
    conn.commit()
    log_audit(deleted_by, st.session_state.username, 'DELETE_USER', 'user', user_id, f"Deleted user ID: {user_id}")
    conn.close()
    return True
...
def save_member(data, profile_picture=None, created_by=None):
    """Save member with enhanced church position fields and school info"""
    conn = sqlite3.connect('kumasi_yoka_registration.db')
    c = conn.cursor()
    ...
    c.execute('''INSERT INTO members (
            ...
            programme, courses, hall_or_hostel, form
              ...
              (data['official_name'], data['date_of_birth'], data['age'], data['residence'], 
               normalize_phone(data['active_phone']), data['email'], profile_pic_bytes,
               ...
               data['has_church_position'], data['church_position_status'], data['church_position_type'], 
               data['church_position_name'], data['church_position_duration'],
               data['work_status'], data['work_type'], data['work_name'], data['work_position'], 
               data['work_location'], data['work_experience_years'],
...
def update_member(member_id, data, updated_by, profile_picture=None):
    ...
    if profile_picture:
        ...
        c.execute('''UPDATE members SET
                 ...
                 WHERE id = ?''',
              (data['official_name'], data['date_of_birth'], data['age'], data['residence'], 
               normalize_phone(data['active_phone']), data['email'],
               data['school_name'], data['school_level'], data['school_class'], data['school_house'],
               data['residence_status'], data['residence_name'], data['church_branch'], branch_id,
               data['yoka_hall'], data['youth_camps_attended'],
               data['has_church_position'], data['church_position_status'], data['church_position_type'], 
    ...
    for idx, row in df.iterrows():
        try:
            data = {
                ...
                'youth_camps_attended': int(row.get('youth_camps_attended', 0)),
...
def email_settings_ui():
    ...
    with st.form("email_config_form"):
        ...
        **Email Configuration Tips:**
        ...
        if test_submitted:
            test_email = st.text_input("Test Email Address", key="test_email")
            if test_email:
    ...
    with st.form("registration_form"):
            ...
            if date_of_birth:
                try:
                    ...
                    if calculated_age < 13:
                        st.warning(f"⚠️ Age ({calculated_age}) is below the allowed range (13-30 years). Member must be at least 13 years old.")
                        age = 13
                    elif calculated_age > 30:
                        st.warning(f"⚠️ Age ({calculated_age}) is above the allowed range (13-30 years). Member must be at most 30 years old.")
                        age = 30
                    else:
                        age = calculated_age
                except:
        ...
        if submitted:
            ...
            if date_of_birth:
                calculated_age = calculate_age_from_dob(date_of_birth)
                if calculated_age < 13:
                    st.error(f"Member must be at least 13 years old. Current age: {calculated_age}")
                    return
                elif calculated_age > 30:
                    st.error(f"Member must be at most 30 years old. Current age: {calculated_age}")
                    return
            ...
            if phone_errors:
                ...
                required_fields = [official_name, residence, active_phone, school_name, residence_status, yoka_hall, gender]
                if school_level:
                    required_fields.append(school_level)
                if church_branch:
                    required_fields.append(church_branch)
                
                has_parent_guardian = (mother_name and mother_phone) or (father_name and father_phone) or (guardian_name and guardian_phone)
                
                if all(required_fields) and has_parent_guardian:
                    ...
                    data = {
                        ...
                        'age': calculate_age_from_dob(date_of_birth) if date_of_birth else age,
                        'residence': residence,
                        ...
                        'programme': programme if school_level == "Tertiary" else "",
...
def view_members_ui():
    ...
    if not members_df.empty:
        ...
        with col5:
            st.metric("With Photos", members_df['profile_picture'].notna().sum())
                ...
                if member:
                    ...
                    with tab1:
                        ...
                        with col2:
                            ...
                            st.write(f"**School Level:** {member[10] if len(member) > 10 else 'N/A'}")
                            st.write(f"**School:** {member[12] if len(member) > 12 else 'N/A'}")
                            st.write(f"**Class/Form:** {member[14] if len(member) > 14 else 'N/A'}")
                        ...
                        if len(member) > 20 and member[20]:
                            ...
                            st.write(f"**Position:** {member[23] if len(member) > 23 else 'N/A'}")
                        ...
                        with st.form("edit_member_form"):
                            ...
                            with col1:
                                ...
                                edit_gender = st.selectbox("Gender", ["Male", "Female", "Other"], 
                                                          index=["Male", "Female", "Other"].index(member[55]) if len(member) > 55 and member[55] in ["Male", "Female", "Other"] else 0)
                            ...
                            with col2:
                                edit_school = st.text_input("School", member[12] if len(member) > 12 else "")
                                edit_hall = st.text_input("YoKA Hall", member[18] if len(member) > 18 else "")
                                edit_camps = st.number_input("Youth Camps Attended", 
                                                            value=int(member[19]) if len(member) > 19 and member[19] else 0,
                                                            min_value=0, max_value=20)
                                new_profile_picture = st.file_uploader("Update Profile Picture", type=['jpg', 'jpeg', 'png'])
                            
                            col3, col4 = st.columns(2)
                            with col3:
                                edit_mother_name = st.text_input("Mother's Name", member[37] if len(member) > 37 else "")
                                edit_mother_phone = st.text_input("Mother's Phone", member[38] if len(member) > 38 else "")
                            with col4:
                                edit_father_name = st.text_input("Father's Name", member[40] if len(member) > 40 else "")
                                edit_father_phone = st.text_input("Father's Phone", member[41] if len(member) > 41 else "")
                            ...
                            if st.form_submit_button("💾 Save Changes"):
                                if edit_phone and not validate_ghana_phone(edit_phone):
                                    ...
                                    edit_data = {
                                        ...
                                        'school_level': member[10] if len(member) > 10 else "SHS",
                                        'school_class': member[14] if len(member) > 14 else "",
                                        'school_house': member[15] if len(member) > 15 else "",
                                        'residence_status': member[13] if len(member) > 13 else "Day Student",
                                        'residence_name': member[17] if len(member) > 17 else "",
                                        'church_branch': member[16] if len(member) > 16 else "",
                                        'yoka_hall': edit_hall,
                                        'youth_camps_attended': edit_camps,
                                        'has_church_position': member[20] if len(member) > 20 else 0,
                                        'church_position_status': member[21] if len(member) > 21 else "",
                                        'church_position_type': member[22] if len(member) > 22 else "",
                                        'church_position_name': member[23] if len(member) > 23 else "",
                                        'church_position_duration': member[24] if len(member) > 24 else "",
                                        'work_status': member[25] if len(member) > 25 else "",
                                        'work_type': member[26] if len(member) > 26 else "",
                                        'work_name': member[27] if len(member) > 27 else "",
                                        'work_position': member[28] if len(member) > 28 else "",
                                        'work_location': member[29] if len(member) > 29 else "",
                                        'work_experience_years': member[30] if len(member) > 30 else 0,
                                        'is_diaspora': member[31] if len(member) > 31 else 0,
                                        'diaspora_country': member[32] if len(member) > 32 else "",
                                        'diaspora_status': member[33] if len(member) > 33 else "",
                                        'diaspora_job': member[34] if len(member) > 34 else "",
                                        'diaspora_school': member[35] if len(member) > 35 else "",
                                        'diaspora_education_level': member[36] if len(member) > 36 else "",
                                        'mother_name': edit_mother_name,
                                        'mother_phone': edit_mother_phone,
                                        'mother_occupation': member[39] if len(member) > 39 else "",
                                        'father_name': edit_father_name,
                                        'father_phone': edit_father_phone,
                                        'father_occupation': member[42] if len(member) > 42 else "",
                                        'guardian_name': member[43] if len(member) > 43 else "",
                                        'guardian_phone': member[44] if len(member) > 44 else "",
                                        'guardian_relationship': member[45] if len(member) > 45 else "",
                                        'guardian_occupation': member[46] if len(member) > 46 else "",
                                        'gender': edit_gender,
                                        'emergency_contact_name': member[56] if len(member) > 56 else "",
                                        'emergency_contact_phone': member[57] if len(member) > 57 else "",
                                        'medical_conditions': member[58] if len(member) > 58 else "",
                                        'talents': member[53] if len(member) > 53 else "",
                                        'interests': member[54] if len(member) > 54 else "",
                                        'programme': member[59] if len(member) > 59 else "",
                                        'courses': member[60] if len(member) > 60 else "",
                                        'hall_or_hostel': member[61] if len(member) > 61 else "",
                                        'form': member[62] if len(member) > 62 else ""
                                    }
                                    try:
...
def user_management_ui():
    ...
    with tab1:
        ...
        if not users_df.empty:
            st.dataframe(users_df[['id', 'username', 'full_name', 'email', 'phone', 'role', 'assigned_branch', 'is_active']], 
                        use_container_width=True)
